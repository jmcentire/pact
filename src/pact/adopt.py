...
def _generate_python_smoke_tests(analysis: CodebaseAnalysis) -> dict[str, str]:
    ...
    for sf in analysis.source_files:
        ...
        lines.append('    """Module imports without error."""')
        ...
        for func in sf.functions:
            ...
            lines.append("    assert callable(fn)")
            lines.append("")
...
def _generate_rust_smoke_tests(analysis: CodebaseAnalysis) -> dict[str, str]:
    ...
    for sf in analysis.source_files:
        ...
        lines.append("        // Verify that the module compiles and is reachable.")
        if module_path != "crate":
            lines.append(f"        use {use_prefix};")
        lines.append("        // Module compiled successfully.")
        lines.append("    }")
        lines.append("")
...
def _generate_typescript_smoke_tests(analysis: CodebaseAnalysis) -> dict[str, str]:
    ...
    for sf in analysis.source_files:
        ...
        # Import test
        lines.append("  it('imports without error', async () => {")
        lines.append(f"    const mod = await import('../src/{import_path}')")
        lines.append("    expect(mod).toBeDefined()")
        lines.append("  })")
        lines.append("")
...
async def adopt_codebase(
    ...
    # Build plan for prioritization (side effects only — populates internal caches)
    plan_test_generation(analysis, complexity_threshold, skip_covered=False)
