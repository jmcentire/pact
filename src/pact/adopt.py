...
def generate_smoke_tests(
    ...
    for sf in analysis.source_files:
        ...
        module_path = sf.path.replace("/", ".").replace("\\", ".")
        ...
        # Import test
        lines.append(f"def test_import_{component_id}():")
...
async def adopt_codebase(
    ...
    # Tests go in tests/smoke/ at project root — conventional, visible, committable
    smoke_suites = generate_smoke_tests(analysis, language)
    smoke_dir = project_path / "tests" / "smoke"
    smoke_dir.mkdir(parents=True, exist_ok=True)
    for cid, code in smoke_suites.items():
        (smoke_dir / f"test_{cid}.py").write_text(code)
